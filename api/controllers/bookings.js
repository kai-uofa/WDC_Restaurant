...
const Bookings = {
  async getActiveBookingsM(req, res) {
    const resId = await db
      .query('SELECT restaurant_id FROM Managers WHERE email = ?', [req.decoded.email])
      .catch(console.error);
...
WHERE Bookings.restaurant_id = ? AND Bookings.status = 1';
  ...
  async getActiveBookings(req, res) {
    const cusId = await db
      .query('SELECT customer_id FROM Customers WHERE email = ?', [req.decoded.email])
      .catch(console.error);
...
WHERE Bookings.customer_id = ? AND Bookings.status = 1';
  ...
  async updateBookingStatus(req, res) {
    if (req.body !== undefined) {
      await db
        .query('UPDATE Bookings SET status = ? WHERE booking_id = ?', [req.body.status, req.body.booking_id])
        .catch(console.error);
    } else {
  ...
  async updateBookingDetails(req, res) {
    if (req.body !== undefined) {
      const updateInfo = 'UPDATE Bookings \
SET date = ?, no_of_people= ?, start_time=? \
WHERE booking_id = ?';

      db.query(updateInfo, [req.body.date, req.body.guests, req.body.time, req.body.booking_id])
        .then(() => {
          res.sendStatus(200);
        })
        .catch(console.error);
